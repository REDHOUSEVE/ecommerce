    ...
    def process_request(self, request):
        ...
        if user_is_authenticated and not user_is_superuser and not user_has_edly_organization_access(request):
            ...
            if request.path != '/logout':
                logout_url = '{lms_url}/logout'.format(lms_url=settings.LMS_ROOT_URL)
                logger.exception('Logout frontend URL %s for site %s.' % (settings.FRONTEND_LOGOUT_URL, request.site))
                logger.exception('Logout URL made using LMS ROOT URL %s for site %s.' % (logout_url, request.site))
